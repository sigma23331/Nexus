...
- **认证方式**: 请求头携带 `Authorization: Bearer {token}`（通过登录接口获得，当前支持手机号验证码与手机号+密码两种方式，微信方式预留）
...
```json
{
  "code": 200,        // 200:成功, 400:请求参数错误, 401:鉴权失败, 403:禁止访问, 404:资源不存在, 429:限流, 500:服务器错误
  "message": "描述信息",
  "data": null        // 具体返回数据
}
```
...
- **返回 data**:
  ...
  ```
- **错误**: 400 手机号格式错误 / 手机号或密码错误 / 该账号未设置密码
...
- **错误**:
  - 400: 请求体必须是有效的JSON / `phone` 字段不能为空
  - 429: 发送频率过高（同一手机号 60 秒内 1 次）
...
- **说明**:
  - 需先通过 `2.2.1` 获取验证码。
  - 当前实现仅接受“登录场景”验证码；如果是新手机号，需先走 `2.1.1 /auth/register` 完成注册。
- **入参**:
  ```json
  {
    "phone": "13800138000",   // required
...
- **返回 data**:
  ...
  ```
- **错误**:
  - 400: 未找到验证码 / 验证码错误或过期 / 该验证码不是登录场景
...
- **返回 data**:
  ...
  ```
- **错误**:
  - 400: 未找到验证码 / 验证码错误或过期 / 手机号已被其他用户绑定
  - 401: token 无效

#### 接口解读
*   **2.2.1 / 2.2.2** 是给**游客/未登录用户**用的，用于通过手机号验证码进入系统。
    *   **当前实现说明**：`2.2.2 /auth/sms/login` 按“登录场景”校验验证码；若手机号尚未完成注册流程，需要先走 `2.1.1 /auth/register`。
...
3.  **防刷机制**：当前实现为同一手机号 **60 秒内最多 1 次**（`429`）。
4.  **后端动作**：生成临时验证记录（如6位数字），先存入服务端临时缓存并调用短信网关发给用户手机。当前开发实现为进程内存存储（`_sms_store`），生产建议改为 Redis。此时数据库里的用户信息还没有任何变化。

##### 2.2.4 确认绑定手机号 (`POST /auth/sms/bind/confirm`)
这个接口是落库动作：
1.  **逻辑闭环**：前端提交：刚才收验证码的手机号 + 6位验证码。
2.  **后端处理**：
    *   **验证验证码**：检查验证码是否匹配且未过期；若未找到验证码记录，当前实现返回 `400`。
    *   **覆盖逻辑**：
...
- **接口**: `POST /auth/wechat/login`（预留，当前后端未接入该路由）
- **入参**:
  ```json
  {
...
- **预期返回 data**（设计参考）:
  ...
  ```
- **当前状态**: 接口未实现，当前后端未注册该路由，直接调用通常会返回 `404 Not Found`。
...
- **手机号为主**: 当前已实现 `2.2.1`、`2.2.2` 以及绑定手机号相关接口 `2.2.3`、`2.2.4`。
- **辅助登录**: 当前已实现 `2.1.1`（手机号+验证码+密码注册）和 `2.1.2`（手机号+密码登录）。
- **绑定手机号逻辑**:
  - 用户必须已登录。
  - 一个手机号只能绑定一个用户。
...
- **入参**:
  ...
  ```json
  {
    ...
    "sourceId": "ans_1002",         // required, 答案ID 或 运势记录ID
...
```
src/mocks/
  auth_wechat_login.json    // POST /auth/wechat/login 响应
  fortune_today.json
  fortune_trend.json
  answer_ask.json
