...
- **返回 data**:
  ```json
  {
    ...
    "health": "注意作息",
    "wealth": "谨慎消费",
    "gua_meaning_lines": ["火土相生", "宜主动求进"], // 卦意两行文案（后端可控，基于分数+趋势）
    "lucky_hour_name": "午时",                      // 开运时辰名称（后端可控，基于分数分档）
    "lucky_hour_range": "11:00-13:00"              // 开运时辰时间段（后端可控，基于分数分档）
  }
  ```
...
- **前端展示规则（看板）**:
  - 当用户历史记录不足 7 天时：按“第 1 天、第 2 天...”从左到右逐格填充，未发生的天数显示为空位（不连线）。
  - 当历史记录达到 7 天及以上时：切换为“最近 7 天”滚动窗口，即第 1 格=6 天前，第 7 格=今天。
- **返回 data**:
...
- **接口**: `DELETE /plaza/card/{cardId}`
- **说明**: 仅卡片作者可删除；删除后关联点赞记录会一并删除。
- **路径参数**:
  - `cardId`: required, 卡片ID
- **返回 data**:
